...
def getFilterTransmission(binseV, draw, thicknesses):
    ...
    filters[:] = (
        filterBe200 ** (thicknesses[2] / 200)
        * filterAl100 ** (thicknesses[1] / 100)
        * filterCH100 ** (thicknesses[0] / 100)
        * filterCu1 ** (thicknesses[3] / 1)
        * filterTi5 ** (thicknesses[4] / 5)
    )
...
def loadFilters(binseV, draw):
    ...
    filters[0, :] = (
        ...
        * filterAl100 ** (21 / 100)
        * filterCH100 ** (21 / 100)
    )  # what goes into pixis, without BFF, with profiler
...
def normalize(spec, just_positive=0):
    if not just_positive:
        nm = np.nanmax(np.abs(spec))
    else:
        s2 = spec * 1
        s2[s2 < 0] = 0
        nm = np.nanmax(np.abs(s2))
    if nm != 0:
        spec = spec / nm
    return spec
...
def plot_mark_at_line(x, ll, ysize=10, lw=1, darkening=0.5, bottom=None, color=None):
    xax = ll[0]._x
    curve = ll[0]._y
    if color is None:
    ...
    if np.isfinite(ceiling):
        if bottom is None:
            ceiling + ysize
        plt.plot([x, x], [bottom, ceiling - ysize], color=color, lw=lw)
...
def text(
    ...
    if coords == "data":
        outside = 0
        if y < np.min(yl) or y > np.max(yl):
            outside = 1
        if x < np.min(xl) or x > np.max(xl):
            outside = 1
        if outside:
            ll(
                verbose,
                "This is outside ({:.2e},{:.2e}):{:} {:}".format(x, y, text, comment),
            )
            return []
    ax = plt.gca()
    if coords == "data":
...
def vline(ll, x, y0=0, color="-", linewidth=1, darkening=1, label=""):
    xl = plt.xlim()
    if color == "-":
        color = ll[0].get_color()
    color = adjust_lightness(color, darkening)
...
def convolve_gauss(vals, fwhm, unitsperpoint=1, debug=0, nan_ends=1, keep_ends=0):
...
def draw_poisson(
    ...
    if np.size(multiplier) > 1:
        _xax2, multiplier = bin_spectrum(xax, multiplierIn, binsize)
...
def drawRect(rect, color=[1, 1, 1], lw=1, label="", fs=24):
    ...
    if label != "":
        ...
        ty = (r[3] - r[2]) / 2 + r[2]
        plt.text(
            tx, ty, label, color=color, fontsize=fs, backgroundcolor=[1, 1, 1, 0.5]
        )
...
def write_table(table, filename, names=0, skip_nans=1):
    ...
    if names != 0:
        ascii.write(
            table, filename, overwrite=True, format="commented_header", names=names
        )
    else:
        ascii.write(table, filename, overwrite=True, format="commented_header")


def center_of_mass(profile):
    coords = np.arange(np.size(profile))
    return weighted_mean(coords, profile)
...
def draw_horizontal_edges(
    centralY, spectralHeight, x0=0, x1=2000, color="w", linewidth=0.5
):
    plt.plot(
        [x0, x1],
        [centralY - spectralHeight, centralY - spectralHeight],
        color=color,
        linewidth=linewidth,
    )
    plt.plot(
...
def grant_all(path, verbose=1):
    ...
    try:
        ...
        ll(verbose, "couldnt change file permission")
...
def gaussfit(
    xax, data, fitmin=8970, fitmax=9100, debug=0, initpars=[], plotoffset=-100
):
    sel = (xax > fitmin) * (xax < fitmax)
    x2 = xax[sel]
    d2 = data[sel]
...
def gauss_l_power(xa, a0, xb, amp, x0, fwhm, power):
    ...
    for i, x in enumerate(xa):
        ret[i] = (
            a0
            + xb * (x - x0)
            + amp * np.exp(-((((x - x0) ** 2) / (2 * sigma**2)) ** power))
        )
    return ret


def gaussfit_l_power(
    xax,
    data,
    fitmin=8970,
    fitmax=9100,
...
def mkdir(fn, verbose=1):
    if not os.path.isdir(fn):
        ll(verbose, "Directory {:s} created.".format(fn))
        os.makedirs(fn)
        return 1
    else:
        ll(verbose, "Directory {:s} already exists.".format(fn))
        return 0
...
def update_shot_params(shot, p, silent=1):
    ...
    for ci, val in enumerate(row):
        ...
        if cname not in p.keys():
            if cname != "comment":
                if not silent:
                    print("this column is wrong: " + cname)
        else:
            if np.size(cnames) == 1:
                if not silent:
                    print("Updating param '{:s}' to value '{:}'.".format(cname, val))
                p[cname] = val
            else:
                if not silent:
                    print(
                        "Updating param '{:s}'/'{:s}' to value '{:}'.".format(
                            cname, cnames[1], val
                        )
                    )
...
def update_progress(progress, maxi, mini=0, style=0, barLength=30):
    ...
    if style:
        ...
        yes = "*"
        no = "-"
    text = "\r [{0}] {1:2,.0f}/{2:2,.0f} {3}".format(
        yes * block + no * (barLength - block), progress, maxi, status
    )
...
def fit_composition(
    xax, spec, init_pars, xrange, maxiter=30, draw_progress=0, free_pars=[]
):
    ...
    for i in np.arange(numsteps):
        ...
        update_progress(i * 1.0, numsteps * 1.0)
    ret_pars = np.reshape(pars, real_shape)
    if draw_progress:
        plt.legend()
    return ret_pars, residuals
...
def ddval(ip, key1, key2, default1=0, default2=0):
    if key1 not in ip.keys():
        return default1
    if key2 not in ip[key1].keys():
        return default2
    else:
        return ip[key1][key2]
...
def ll(verbose, *messages, ticking=0):
    if verbose <= 0:
        return
    global print_levels
...
def qclim(minq, maxq, data):
    plt.clim(np.quantile(data.flatten(), [minq, maxq]))
...
def pcolor(
    ...
    dt = np.transpose(
        data
    )  # This transpotion is the weirdes thing ever; Basically I made this routine mainly to have this consistently transposed
...
def subprocess(command, cwd="", verbose=0, print_output=0):
    ...
    ll(verbose, "##### Starting process " + command)
    ...
    ll(verbose, "      Exit process with code {:}".format(code))
    return code
