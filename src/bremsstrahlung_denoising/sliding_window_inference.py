import colorscheme as rofl
import matplotlib.colors as colors
import matplotlib.pyplot as plt
import mmmUtils as mu
import numpy as np
import torch
from dataset import quantile_normalization


def patchwise_prediction(
    noisy_input,
    model,
    patch_size=128,
    stride=128,
    ...
    # TODO: this hack needs to go, as you always go for tile, the function parameter makes no sense anymore
...
def weighted_sliding_window_inference(noisy_signal, model):
...
def plot_prediction(
    noisy_signal, prediction, runNo, case, fn, draw_output, ax, noise_cmax
):
    ...
    mu.figure(12, 5)
    plt.subplot(231)
    plt.imshow(noisy_signal.transpose(), norm=colors.LogNorm(), cmap=rofl.cmap())
    plt.colorbar()
    plt.clim(cl)
    plt.axis(ax)
    plt.axis("off")
    ...
    plt.subplot(232)
    plt.imshow(prediction.transpose(), norm=colors.LogNorm(), cmap=rofl.cmap())
    plt.colorbar()
    plt.clim(cl)
    plt.axis("off")
    ...
    plt.axis(ax)
    plt.subplot(233)
    plt.imshow(pure_noise.transpose(), cmap=rofl.cmap())
    plt.colorbar()
    ...
    mu.savefig("out/{:}_denoised_{:}".format(fn, case))
